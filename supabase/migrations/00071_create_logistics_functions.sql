-- 创建发货订单
CREATE OR REPLACE FUNCTION create_shipping_order(
  p_asn_id BIGINT,
  p_shipper_name TEXT,
...
BEGIN
  ...
  INSERT INTO shipping_orders (
    order_code, asn_id, shipper_name, shipper_contact, shipper_address,
    consignee_name, consignee_contact, consignee_address,
    carrier, estimated_ship_date, items, status, tenant_id, created_by
  ) VALUES (
    v_order_code, p_asn_id, p_shipper_name, p_shipper_contact, p_shipper_address,
...
-- 记录物流异常事件
CREATE OR REPLACE FUNCTION create_logistics_exception_event(
  p_tracking_id BIGINT,
  p_event_type TEXT,
  p_description TEXT,
  p_location TEXT,
...
-- 检测物流异常（超时无更新）
CREATE OR REPLACE FUNCTION detect_logistics_timeout_exceptions(
