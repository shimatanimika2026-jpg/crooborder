...
DECLARE
  ...
  v_expected_sn TEXT;
BEGIN
  -- ✅ 新增：步骤0 - 校验关键件 serial_number 与传入 SN 一致
  FOR v_part IN SELECT * FROM jsonb_array_elements(p_parts)
  LOOP
    -- 查询真实来料的 serial_number
    SELECT serial_number, part_type INTO v_receiving_item
    FROM receiving_record_items
    WHERE id = (v_part->>'receiving_record_item_id')::BIGINT;
    
    -- 对关键件进行 serial_number 校验
    IF v_receiving_item.part_type IN ('control_box', 'teaching_pendant', 'main_board', 'servo_driver', 'power_supply', 'controller') THEN
      -- 关键件必须有 serial_number
      IF v_receiving_item.serial_number IS NULL THEN
        RAISE EXCEPTION '关键件来料缺少序列号 [% - ID:%]，不能用于组装', 
          v_receiving_item.part_type, 
          v_part->>'receiving_record_item_id';
      END IF;
      
      -- 根据 part_type 确定期望的 SN
      v_expected_sn := NULL;
      IF v_receiving_item.part_type = 'control_box' THEN
        v_expected_sn := p_control_box_sn;
      ELSIF v_receiving_item.part_type = 'teaching_pendant' THEN
        v_expected_sn := p_teaching_pendant_sn;
      ELSIF v_receiving_item.part_type = 'main_board' THEN
        v_expected_sn := p_main_board_sn;
      END IF;
      
      -- 校验 serial_number 一致性
      IF v_expected_sn IS NOT NULL AND v_receiving_item.serial_number != v_expected_sn THEN
        RAISE EXCEPTION '关键件序列号不一致 [%]: 来料SN=%, 传入SN=%', 
          v_receiving_item.part_type,
          v_receiving_item.serial_number,
          v_expected_sn;
      END IF;
    END IF;
  END LOOP;
