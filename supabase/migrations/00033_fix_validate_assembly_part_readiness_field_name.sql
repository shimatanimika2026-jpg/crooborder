...
-- 修正validate_assembly_part_readiness函数的字段名
...
BEGIN
  ...
  WHERE robot_sn = NEW.finished_product_sn AND part_type = 'control_box'
  ORDER BY installed_at DESC
  LIMIT 1;

  -- 如果找不到控制箱映射,阻断
  IF v_control_box_part_no IS NULL THEN
    RAISE EXCEPTION '控制箱未建立来料映射关系,不可上线 (finished_product_sn: %)', NEW.finished_product_sn;
  END IF;
  ...
  FROM assembly_part_material_mapping
  WHERE robot_sn = NEW.finished_product_sn AND part_type = 'teaching_pendant'
  ORDER BY installed_at DESC
  LIMIT 1;

  -- 如果找不到示教器映射,阻断
  IF v_teaching_pendant_part_no IS NULL THEN
    RAISE EXCEPTION '示教器未建立来料映射关系,不可上线 (finished_product_sn: %)', NEW.finished_product_sn;
  END IF;
  ...
  IF NEW.main_board_sn IS NOT NULL THEN
    ...
    FROM assembly_part_material_mapping
    WHERE robot_sn = NEW.finished_product_sn AND part_type = 'main_board'
    ORDER BY installed_at DESC
    LIMIT 1;

    -- 如果找不到主板映射,阻断
    IF v_main_board_part_no IS NULL THEN
      RAISE EXCEPTION '主板未建立来料映射关系,不可上线 (finished_product_sn: %)', NEW.finished_product_sn;
    END IF;
