...
-- 修正check_part_assembly_readiness函数的处置单检查逻辑
...
BEGIN
  ...
  -- 4. 检查是否有未完成的处置单(pending或rejected才阻断)
  IF EXISTS (
    SELECT 1 FROM incoming_material_dispositions
    WHERE part_no = p_part_no
      AND batch_no = p_batch_no
...
COMMENT ON FUNCTION check_part_assembly_readiness IS '检查零件是否可以上线组装(基于料号+批次号,优先选择无差异记录)';
